<template>
  <div class="page-search mx-16">
    <header class="title-search">
      <h1>Folders "{{ search }}"</h1>
    </header>
    <div class="link-bottom-image">
      <v-row class="mt-9">
        <NuxtLink :to="`/search/${search}`"> Photo </NuxtLink>

        <NuxtLink :to="`/search/${search}/folders`">
          Folders <span class="ml-1" style="color: rgb(48, 48, 48); font-size: 15px">{{ totalData }}</span></NuxtLink
        >
    ...
    <div class="mt-12">
      ...
      <CollectionFolders :dataFolders="searchData" />
    </div>
  </div>
</template>
...
try {
  const { data } = await useFetch(`${url}/search/${search}/folders`);
