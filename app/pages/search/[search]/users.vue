<template>
  <div class="page-search mx-16">
    ...
    <div class="link-bottom-image">
      <v-row class="mt-9">
        <NuxtLink :to="`/search/${search}`"> Photo </NuxtLink>

        <NuxtLink :to="`/search/${search}/folders`"> Folders</NuxtLink>
        <NuxtLink :to="`/search/${search}/users`">
          Users <span class="ml-1" style="color: rgb(48, 48, 48); font-size: 15px">{{ totalData }}</span></NuxtLink
        >
      </v-row>
    </div>
    ...
    <div class="mt-12">
      <v-row v-if="skeletonLoaders">
        <v-col v-for="index in totalData" :key="index" cols="12" sm="1" md="3">
          <v-skeleton-loader max-width="700" type="image@3"></v-skeleton-loader>
        </v-col>
      </v-row>
      <CollectionUsers v-else :dataUsers="searchData" />
    </div>
  </div>
</template>
...
try {
  const { data } = await useFetch(`${url}/search/${search}/users`);
