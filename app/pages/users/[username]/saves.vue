<template>
  <div class="page-user">
    ...
    <div class="link mt-10 ml-16">
      <NuxtLink :to="`/users/${username}`"> Photos <span class="ml-1" style="color: rgb(48, 48, 48); font-size: 15px"></span></NuxtLink>
      <NuxtLink :to="`/users/${username}/folders`"> Folders</NuxtLink>
      <NuxtLink :to="`/users/${username}/saves`">
        Saves <span class="ml-1" style="color: rgb(48, 48, 48); font-size: 15px">{{ dataImages.length }}</span></NuxtLink
      >
...
try {
  const { data } = await useFetch(`${url}/user/${username}/saves`);
